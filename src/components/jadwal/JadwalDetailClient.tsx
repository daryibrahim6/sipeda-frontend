 ...
 * Owns the single schedule state + Supabase Realtime subscription,
 * so both the quota bar AND registration form stay in sync.
 */
export function JadwalDetailClient({ schedule: initialSchedule }: { schedule: Schedule }) {
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sticky top-20">
                    {isCancelled ? (
                        ...
                        <>
                            ...
                            <RegisterForm schedule={schedule} onRegistrationSuccess={refreshSchedule} />
                        </>
                    )}
                </div>
            </div>
